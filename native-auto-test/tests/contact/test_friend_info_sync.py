...
def _restore_login(device, user_id: str, *, password: str = "1") -> None:
    """恢复 session 登录态，避免当前 case 失败污染后续 real_e2e 用例。"""
    device.call(
        "Client",
        Cmd.login.value,
        info={"userId": user_id, "pwdOrToken": password, "isPassword": True},
    )
    try:
...
def test_friend_info_auto_sync_after_login(device_a, device_b, assert_api, user_a, user_b):
    ...
    2. 清理残留事件后，deviceA 和 deviceB 分别使用原账号调用 Client.login；
    3. 校验两端登录后均收到 onFriendStartSync 和 onFriendSyncFinished，并在 finally 中恢复登录态。
    """
    describe_case_steps(
        "1. 在 session lifecycle 专项中让 deviceA 和 deviceB 先执行 Client.logout，制造双端重新登录场景；\n"
        "2. 清理残留事件后，deviceA 和 deviceB 分别使用原账号调用 Client.login；\n"
    ...
    try:
        ...
        _restore_login(device_a, user_a)
        _restore_login(device_b, user_b)
...
def test_friend_info_sync_on_peer_metadata_change(topology, assert_api):
    ...
    # 清理好友关系；本 case 已完成业务断言，删除回调不作为本场景证据。
