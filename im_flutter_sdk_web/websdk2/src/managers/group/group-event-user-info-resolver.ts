...
export interface GroupUserInfoFetchParams {
  readonly userIds: ReadonlyArray<string>;
}

export type GroupUserInfoFetcher = (
  params: GroupUserInfoFetchParams
) => Promise<ReadonlyArray<UserInfo>>;

interface GroupUserInfoResolverOptions {
  readonly cacheManager: Pick<CacheManager, 'getUserInfoSummaries'> | null;
  readonly fetchUserInfos?: GroupUserInfoFetcher;
}
...
export const buildMinimalGroupUserInfo = (userId: string): UserInfo => ({
  userId,
});

export const mergeGroupUserInfo = (
...
export const resolveGroupUserInfoMap = async (
  userIds: ReadonlyArray<string>,
  options: GroupUserInfoResolverOptions
  ...
  for (const userId of normalizedUserIds) {
    if (!resolved.has(userId)) {
      resolved.set(userId, buildMinimalGroupUserInfo(userId));
    }
  }
...
export const resolveGroupUserInfos = async (
  userIds: ReadonlyArray<string>,
  options: GroupUserInfoResolverOptions
): Promise<ReadonlyArray<UserInfo>> => {
  const normalizedUserIds = dedupeUserIds(userIds);
  const userInfoMap = await resolveGroupUserInfoMap(normalizedUserIds, options);
  return normalizedUserIds.map(userId => mergeGroupUserInfo(undefined, userInfoMap.get(userId), userId));
};
